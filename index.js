...
function TodoFooter(todos, onChange) {
    const container = document.createElement("div");
    const comleted = todos.filter(todo => todo.completed === true).length;
    container.innerHTML = `
        <span>${comleted} / ${todos.length} Complated</span>
...
function App() {
    ...
    let todos = [
        {label: "Tikon hac uti ?", completed: false},
        {label: "Tikon qayli ?", completed: false},
        {label: "Tikon qni ?", completed: false}
    ];
